...
public class DisplayController {
	...
	private void handleSetupMode() {
		byte numberPressed = findNumberPressed();
		if (numberPressed != NO_PRESS) {
			shiftDigitsAndSetLast(numberPressed);
		}
	}

	private byte findNumberPressed() {
	...
	private void shiftDigitsAndSetLast(byte numberPressed) {
	...
	private void handleCookingMode() {
		ticks++;
		if (secondElapsed() && timeToCook() != 0) {
			decrementDigitsOrRollOver();
		}
	}

	private void decrementDigitsOrRollOver() {
